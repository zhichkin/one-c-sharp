...
namespace Zhichkin.Hermes.Model
{
    public partial class Request
    {
        ...
        {
            #region " SQL "
            private const string SelectCommandText = @"SELECT [version], [name], [namespace], [owner], [parse_tree], [request_type], [response_type] FROM [hermes].[requests] WHERE [key] = @key";
            private const string InsertCommandText =
                @"DECLARE @result table([version] binary(8)); " +
                @"INSERT [hermes].[requests] ([key], [name], [namespace], [owner], [parse_tree], [request_type], [response_type]) " +
            ...
            private const string UpdateCommandText =
                ...
                @"UPDATE [hermes].[requests] SET [name] = @name, [namespace] = @namespace, [owner] = @owner, [parse_tree] = @parse_tree, " +
                ...
                @"IF (@rows_affected = 0) " +
                @"BEGIN " +
                @"  INSERT @result ([version]) SELECT [version] FROM [hermes].[requests] WHERE [key] = @key; " +
                @"END " +
                @"SELECT @rows_affected, [version] FROM @result;";
            private const string DeleteCommandText =
                @"DELETE [hermes].[requests] WHERE [key] = @key " +
