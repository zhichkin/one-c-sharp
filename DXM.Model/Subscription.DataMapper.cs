...
namespace Zhichkin.DXM.Model
{
    public partial class Subscription
    {
        public sealed class DataMapper : IDataMapper
        {
            ...
            private const string SelectCommandText = @"SELECT [version], [name], [source], [target] FROM [dxm].[subscriptions] WHERE [key] = @key";
            private const string InsertCommandText =
                @"DECLARE @result table([version] binary(8)); " +
                @"INSERT [dxm].[subscriptions] ([key], [name], [source], [target]) " +
                @"OUTPUT inserted.[version] INTO @result " +
                @"VALUES (@key, @name, @source, @target); " +
            ...
            private const string UpdateCommandText =
                ...
                @"UPDATE [dxm].[subscriptions] SET [name] = @name, [source] = @source, [target] = @target " +
                @"OUTPUT inserted.[version] INTO @result" +
                @" WHERE [key] = @key AND [version] = @version; " +
                @"SET @rows_affected = @@ROWCOUNT; " +
                @"IF (@rows_affected = 0) " +
                @"BEGIN " +
                @"  INSERT @result ([version]) SELECT [version] FROM [dxm].[subscriptions] WHERE [key] = @key; " +
                @"END " +
                @"SELECT @rows_affected, [version] FROM @result;";
            private const string DeleteCommandText =
                @"DELETE [dxm].[subscriptions] WHERE [key] = @key " +
            ...
            void IDataMapper.Select(IPersistent entity)
            {
                ...
                using (SqlConnection connection = new SqlConnection(ConnectionString))
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        e._Source = Factory.New<Entity>(reader.GetGuid(2));
                        e._Target = Factory.New<Entity>(reader.GetGuid(3));
