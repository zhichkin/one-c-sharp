...
namespace Zhichkin.DXM.Model
{
    public partial class Publication
    {
        public sealed class DataMapper : IDataMapper
        {
            ...
            private const string SelectCommandText = @"SELECT [version], [name], [publisher] FROM [dxm].[publications] WHERE [key] = @key";
            private const string InsertCommandText =
                @"DECLARE @result table([version] binary(8)); " +
                @"INSERT [dxm].[publications] ([key], [name], [publisher]) " +
                @"OUTPUT inserted.[version] INTO @result " +
                @"VALUES (@key, @name, @publisher); " +
            ...
            private const string UpdateCommandText =
                ...
                @"UPDATE [dxm].[publications] SET [name] = @name, [publisher] = @publisher " +
                @"OUTPUT inserted.[version] INTO @result" +
                @" WHERE [key] = @key AND [version] = @version; " +
                @"SET @rows_affected = @@ROWCOUNT; " +
                @"IF (@rows_affected = 0) " +
                @"BEGIN " +
                @"  INSERT @result ([version]) SELECT [version] FROM [dxm].[publications] WHERE [key] = @key; " +
                @"END " +
                @"SELECT @rows_affected, [version] FROM @result;";
            private const string DeleteCommandText =
                @"DELETE [dxm].[publications] WHERE [key] = @key " +
            ...
            {
                Publication e = (Publication)entity;
                ...
                using (SqlConnection connection = new SqlConnection(ConnectionString))
                {
                    ...
                    if (reader.Read())
                    {
                        ...
                        e._Publisher = Factory.New<InfoBase>(reader.GetGuid(2));
            ...
            void IDataMapper.Insert(IPersistent entity)
            {
                ...
                using (SqlConnection connection = new SqlConnection(ConnectionString))
                {
                    ...
                    parameter.Direction = ParameterDirection.Input;
                    parameter.Value = (e._Publisher == null) ? Guid.Empty : e._Publisher.Identity;
                    command.Parameters.Add(parameter);
